...
def main(page: ft.Page):
    ...
    SHADOW_COLOR = "#1A000000"  # Negro al 10% de opacidad
    ...
    def mostrar_mensaje(texto, color):
        # CORRECCIÓN: page.open() no existe en esta versión; usar snack_bar + update
        snack = ft.SnackBar(
    ...
    def mostrar_registro():
        ...
        txt_confirmar_pass = ft.TextField(
            label="Confirmar Contraseña", width=320,
            password=True, can_reveal_password=True,
            prefix_icon=ft.Icons.LOCK,
            border_color=ft.Colors.PURPLE_200,
            focused_border_color=ft.Colors.PURPLE
            ...
            ft.Container(
                content=ft.Column(
                    [
                        ...
                        ft.Text("CREAR NUEVA CUENTA", size=28,
                                weight=ft.FontWeight.BOLD, color=ft.Colors.PURPLE_700),
                        ft.Divider(height=20, color=ft.Colors.PURPLE_100),
                        txt_nuevo_usuario, txt_nueva_pass,
                        txt_confirmar_pass, lbl_reg_error,
                        ft.Button(
                            "Registrar", on_click=registrar_usuario,
                            width=320, height=45,
                            bgcolor=ft.Colors.PURPLE, color=ft.Colors.WHITE
                        ),
                        ft.TextButton("Volver al Login",
    ...
    def mostrar_panel_principal():
        ...
        filtro_letras = ft.InputFilter(
            allow=True,
            regex_string=r"[a-zA-ZáéíóúÁÉÍÓÚñÑ ]",
            replacement_string=""
        )
        filtro_numeros = ft.InputFilter(
            allow=True,
            regex_string=r"[0-9]",
            replacement_string=""
        )
        ...
        txt_disciplina = ft.Dropdown(
            label="Disciplina Deportiva", width=220,
            options=[
        ...
        # CORRECCIÓN: ft.ImageFit no existe en esta versión; usar string directamente
        img_perfil = ft.Image(
            src=IMG_DEFECTO, width=110, height=110,
        ...
        contenedor_tabla = ft.Container(
            ...
            height=280, bgcolor=ft.Colors.WHITE,
        ...
        def al_seleccionar_archivo(e: ft.FilePickerResultEvent):
            ...
            if e.files:
                ...
                if ext in ['.jpg', '.jpeg', '.png', '.bmp']:
                    ...
                    mostrar_mensaje(
                        "Formato de imagen inválido (.png, .jpg, .jpeg)",
                        ft.Colors.PINK_400
                    )
            ...
            try:
                ...
                if not alumnos:
                    ...
                    encabezados = ft.Container(
                        content=ft.Row([
                            ft.Text("Foto",       weight=ft.FontWeight.BOLD, size=13,
                                    color=ft.Colors.PURPLE_700, width=60),
                            ft.Text("Matrícula",  weight=ft.FontWeight.BOLD, size=13,
                                    color=ft.Colors.PURPLE_700, width=100),
                            ft.Text("A. Paterno", weight=ft.FontWeight.BOLD, size=13,
                                    color=ft.Colors.PURPLE_700, width=110),
                            ft.Text("A. Materno", weight=ft.FontWeight.BOLD, size=13,
                                    color=ft.Colors.PURPLE_700, width=110),
                            ft.Text("Nombre(s)",  weight=ft.FontWeight.BOLD, size=13,
                                    color=ft.Colors.PURPLE_700, width=130),
                            ft.Text("CURP",       weight=ft.FontWeight.BOLD, size=13,
                                    color=ft.Colors.PURPLE_700, width=150),
                            ft.Text("Teléfono",   weight=ft.FontWeight.BOLD, size=13,
                                    color=ft.Colors.PURPLE_700, width=90),
                            ft.Text("Acciones",   weight=ft.FontWeight.BOLD, size=13,
                                    color=ft.Colors.PURPLE_700, width=100),
                        ], spacing=10),
                        ...
                        def crear_fila(alumno_data):
                            def editar_click(e):
                                ...
                                txt_disciplina.value = (
                                    alumno_data[8] if alumno_data[8] else "NINGUNA"
                                )
                            ...
                            def eliminar_click(e):
                                # CORRECCIÓN: usar dialogo.open en lugar de page.open/page.close
                                dialogo = ft.AlertDialog(
                                    modal=True,
                                    title=ft.Text("Confirmar eliminación"),
                                    content=ft.Text(
                                        f"¿Desea eliminar de forma permanente "
                                        f"al alumno {alumno_data[0]}?"
                                    ),
                                    actions=[
                                        ft.TextButton("Cancelar", on_click=lambda _: cerrar_dialogo()),
                                        ft.Button(
                                            "Eliminar",
                                            on_click=lambda _: confirmar_eliminar(),
                            ...
                            # CORRECCIÓN: fit como string
                            mini_foto = ft.Image(
                                src=foto_src, width=30, height=30,
        ...
        def guardar_alumno(e):
            nonlocal ruta_foto_seleccionada
            if not all([
                txt_matricula.value, txt_apellido_paterno.value,
            ...
            try:
                cursor_db.execute("""
                    INSERT INTO alumnos (
                        matricula, apellido_paterno, apellido_materno,
                        nombre, curp, especialidad, telefono,
                        ciudad_origen, estado, disciplina, foto_ruta
                ...
                mostrar_mensaje("Error: La matrícula o CURP ya existen.", ft.Colors.PINK_400)
        ...
        def actualizar_alumno(e):
            ...
            if not selected_matricula:
                ...
                return
            if not all([
                txt_apellido_paterno.value, txt_apellido_materno.value,
                txt_nombres.value, txt_curp.value,
                txt_especialidad.value, txt_telefono.value,
            ...
            if (ruta_foto_seleccionada
                    and os.path.exists(ruta_foto_seleccionada)
            ...
            try:
                cursor_db.execute("""
                    UPDATE alumnos
                    SET apellido_paterno=%s, apellido_materno=%s,
        ...
        header = ft.Container(
            content=ft.Row([
                ...
                ft.IconButton(
                    ...
                    on_click=lambda _: [
                        setattr(page, "scroll", None), mostrar_login()
                    ]
                )
        ...
        form_card = ft.Container(
            content=ft.Row([
                ft.Column([
                    ...
                    ft.Row([
                        ft.Button("Guardar", on_click=guardar_alumno,
                                  bgcolor=ft.Colors.PURPLE,
                                  color=ft.Colors.WHITE, width=120),
                        ft.Button("Actualizar", on_click=actualizar_alumno,
                                  bgcolor=ft.Colors.PURPLE_300,
                                  color=ft.Colors.WHITE, width=120),
                        ft.Button("Limpiar", on_click=limpiar_formulario,
                                  bgcolor=ft.Colors.PURPLE_100,
                                  color=ft.Colors.PURPLE_700, width=120),
                        ft.Button("Salir", on_click=salir_programa,
                                  bgcolor=ft.Colors.PINK_400,
                                  color=ft.Colors.WHITE, width=120),
                    ], spacing=15),
                    lbl_resultado
